...
exports.getDispatchedData = async (req, res) => {
    try {
        ...
        const { schoolId } = req.params; // Extract schoolId from route parameters

        // Filtering conditions
        const filter = { status: 'delivered' };
        if (startDate) filter.dispatchedAt = { $gte: new Date(startDate) };
        if (endDate) filter.dispatchedAt = { ...filter.dispatchedAt, $lte: new Date(endDate) };
        if (schoolId) filter.schoolId = schoolId; // Apply schoolId filter if provided
        ...
        const response = dispatchedData.reduce((acc, curr) => {
            const date = curr.dispatchedAt.toISOString().split('T')[0];
